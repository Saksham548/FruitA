...
export function DeleteModel({ mod, id }) {
    ...
    const hendleDelete = async () => {
        setLoader(true);
        axios.delete(`https://fruit-ai-five-snowy.vercel.app/faqs/${id}`).then
            (() => {
                setLoader(false);
            ...
            <div onClick={(e) => e.stopPropagation()} className="w-[80%] md:w-[60%] custon-shadow rounded-2xl px-10 bg-white flex flex-col p-2 xxxs:px-6 md:py-8 lg:py-12 py-8 gap-10  modelAnimation" >
                <div className="flex w-full justify-between h-fit items-center relative">
                    <h1 className="md:text-3xl ss:text-2xl xxs:text-xl text-lg sm:mr-5 font-[500] text-[#ff054f]">
                        Confirm delete this FaQ permanently!
                    </h1>
                    <RxCross2
                ...
                <p className="text-[14px] text-[#8f8f8f] font-[500]">
                    <b>NOTE-</b>&nbsp;Once The FaQ is deletey can not be recovered
                    back conform if you want to delete This FaQ permanently...
                </p>
...
export function EditProd({ pro, mod }) {
    ...
    const [productName, setProductName] = useState(pro.title);
    const [proDis, setProDis] = useState(pro.description);
...
console.log(pro);
    ...
    const updateProduct = () => {
        if (productName.trim() === "") {
            ...
            setLoader(true);
            axios.put(`https://fruit-ai-five-snowy.vercel.app/faqs/${pro._id}`, {
                title: productName,
                description: proDis,
            }).then(() => {
                setLoader(false);
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="w-[100%] flex flex-col md:flex-row border-none h-fit justify-center items-center  mt-4">
                    <div className="w-[90%] md:w-[89%] flex flex-col justify-center items-center gap-1 xxxs:gap-6">
...
export function AddProd({ mod }) {
    ...
    const updateProduct = () => {
        if (productName.trim() === "") {
            ...
            setLoader(true);
            axios.post(`https://fruit-ai-five-snowy.vercel.app/faqs`, {
                title: productName,
                description: proDis,
            }).then(() => {
                setLoader(false);
